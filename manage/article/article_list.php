        ...
        <form name='form_add' id='form_add' action='article_add_pass.php' method='post'>
            <table width='100%' border='1' cellspacing='0' cellpadding='0'>
                <tr style='color:red;text-align:center;background-color:yellow;'>
                    <td>article list</td>
                    <td></td>
                    <td><input type="button" value="add article" onclick="window.location.href='article_add.php?'"/></td>
                </tr>
                <tr>
                    <td height='35'>title</td>
                    <td height='35'>content</td>
                ...
                <?php
                    ...
                    $sql="select * from article order by id asc limit $offset,$pagesize";
                    $result=mysql_query($sql);
                    while($row=mysql_fetch_array($result)){
                ?>
                    <tr>
                        <td height='20'><?=$row['title']?></td>
                        <td height='20'><?=$row['content']?></td>
                        <td height='20'><?=$row['comefrom']?></td>
                        <td height='20'><?=$row['pubdate']?></td>
                        <td width='28%'>
                            <input type="button" name="button" id="button" value="modify" onclick="window.location.href='article_modify.php?id=<?=$row['id']?>'" />&nbsp;&nbsp;
                            <input type='button' name='button2' id='button2' value='delete' onclick="window.location.href='article_delete.php?id=<?=$row['id']?>'" />
                        </td>
                    </tr>
                <?php } ?>
                    <tr>
