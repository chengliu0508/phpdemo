    ...
    <body>
        <?php
            ...
            if($title==''){
                ...
                exit;
            }elseif(mysql_num_rows(mysql_query("select * from article where title='".$_POST['title']."'"))>0){
            ...
            if($content==''){
                echo "<script>alert('content can not be null!');history.go(-1)</script>";
                exit;
            }
            $comefrom=$_POST['comefrom'];
            $pubdate=$_POST['pubdate'];
            $sql_add="insert into article(title,content,comefrom,pubdate) values('".$title."','".$content."','".$comefrom."','".$pubdate."')";
            mysql_query($sql_add);
